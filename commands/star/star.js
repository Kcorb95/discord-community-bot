...
module.exports = class StarCommand extends Command {
  ...
  async run(msg, { message }) {
    ...
    if (isAuthor || message.author.id === msg.author.id) {
      return msg.embed({ color: 14365748, description: `${msg.author}, you can't star your own messages.` });
    }
    const hasStarred = await Starboard.hasStarred(message.id, msg.author.id);
    if (hasStarred) {
      return msg.embed({ color: 14365748, description: `${msg.author}, you've already starred this message.` });
    }
