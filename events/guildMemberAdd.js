...
exports.run = async(bot, member) => {
  ...
  if (logs && logs.enabled && logs.channel && (logs.fields ? logs.fields.joins !== false : !logs.fields) && member.guild.channels.has(logs.channel)) {
    ...
    // TODO: say how old the account is, also improve code here, its fucking shit man
	  embed.setColor('#66ff99').setTimestamp(new Date()).setAuthor(`${member.user.username} (${member.user.id})`, member.user.displayAvatarURL);
  ...
  if (welcome && welcome.enabled === true) {
      if (welcome.pm && welcome.pm.enabled === true && welcome.pm.message && member.guild.channels.has(logs.channel)) await member.send(welcome.pm.message.replace(/\[user\]/g, member));
    ...
    if (welcome.public && welcome.public.enabled !== false && welcome.public.message) {
      if (welcome.public.channel && member.guild.channels.get(welcome.public.channel)) {
        ...
        else member.guild.channels.get(welcome.public.channel).send(welcome.public.message.replace(/USER/g, member));
      }
      else {
          await member.guild.owner.send(`You seem to have welcome messages enabled, but not configured properly. A new member joined in\`${member.guild.name}\`but a valid channel is not set! Please set a valid channel for welcome messages in\`${member.guild.name}\`!`);
      }
    }
  }
  // join flairs
  ...
  if (rolestate && rolestate.enabled && rolestate.users && rolestate.users[member.id]) {
    ...
    member.addRoles(roles).then(async() => {
      ...
      const roleNames = rolestate.users[member.id].map(roleid => { // eslint-disable-line
        ...
        await member.guild.owner.send(`\uD83D\uDEAB I do not have permissions to reinstate the roles for ${member} in the server \`${member.guild.name}\`. Here are the roles I remembered for this user:\n\`${roleNames.join(', ').slice(2)}\`.`);
      delete rolestate.users[member.id];
      settings.rolestate = rolestate;
      await settings.save();
    });
