...
const {DEFAULT_VOLUME, GOOGLE_API, MAX_LENGTH, MAX_SONGS, PASSES, SOUNDCLOUD_API} = require('../../settings.json'); // eslint-disable-line max-len
...
module.exports = class PlaySongCommand extends Command {
    ...
    async handleVideo(video, queue, voiceChannel, msg, statusMsg) {
        ...
        if (!queue) {
            ...
            if (!result.embeds[0].description.startsWith('👍')) {
    ...
    async handlePlaylist(playlist, queue, voiceChannel, msg, statusMsg) {
        ...
        for (const video of Object.values(videos)) {
            ...
            if (!queue) {
                ...
                if (!result.embeds[0].description.startsWith('👍')) this.queue.delete(msg.guild.id);
